    ...
    for row in table.find_all("tr"):  # each player
        ...
        for i, cell in enumerate(cells[:-1]):  # last row is a repeat
            ...
            for span in spans:
                ...
                side = (
                    "t"
                    if "mod-t" in span["class"]
                    else "ct"
                    if "mod-ct" in span["class"]
                    else None
                )
                if side:
...
def extract_tables_for_game(soup, gameid, teaminfo):
    ...
    tables_data = []
    tables = game_div.find_all_next(
        "table", {"class": "wf-table-inset mod-overview"}, limit=2
    )
    ...
    for table in tables:
        ...
        if not team_2.find("span", {"class": "mod-ct"}) or not team_1.find(
            "span", {"class": "mod-ct"}
        ):
            return []
        ...
        scoreinfo[f"{teaminfo[team2_name]}c"] = team_2.find(
            "span", {"class": "mod-ct"}
        ).get_text(strip=True)
        scoreinfo[f"{teaminfo[team2_name]}t"] = team_2.find(
            "span", {"class": "mod-t"}
        ).get_text(strip=True)
        scoreinfo[f"{teaminfo[team1_name]}c"] = team_1.find(
            "span", {"class": "mod-ct"}
        ).get_text(strip=True)
        scoreinfo[f"{teaminfo[team1_name]}t"] = team_1.find(
            "span", {"class": "mod-t"}
        ).get_text(strip=True)
            ...
            if date_info != None:
                ...
                if team_info and maps_info:
                    for map_info in maps_info:
                        ...
                        all_game_table_data.extend(
                            [
                                {
                                    "game_id": gameid,
                                    "map_name": map_name,
                                    "date_info": date_info.get("data-utc-ts"),
                                }
                            ]
                        )
